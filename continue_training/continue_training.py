...
def continue_training(model, optimizer, train_data, tokenizer, device, args):
    # 加载之前的模型状态字典
    checkpoint = torch.load(os.path.join(args.model_dir, 'continue_training_model'))

    # 创建新模型实例
    model = torch.load(os.path.join(args.model_dir, 'continue_training_model')).to(device)

    # 加载状态字典
    model.load_state_dict(checkpoint['model_state_dict'], strict=False)
    optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
    start_epoch = checkpoint['epoch'] + 1  # 下一个epoch开始的位置
    ...
    for epoch in range(start_epoch, args.num_epoch + start_epoch):
        ...
        if args.data_parallel and torch.cuda.is_available():
            ...
            }, os.path.join(args.model_dir, 'new_model'))
        else:
            torch.save({
                'epoch': epoch,
...
if __name__ == '__main__':
    ...
    # 继续训练
    continue_training(model, optimizer, train_data, tokenizer, device, args)
