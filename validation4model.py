...
def prepare_data(args, data_path, tokenizer, term='train'):
    ...
    return data  # 直接返回 DataLoader 对象
...
# 配置日志
log_path = 'validation.log'  # 替换成实际的日志文件路径
logging.basicConfig(filename=log_path, level=logging.INFO)

# 创建日志对象
logger = logging.getLogger(__name__)

def validate_model(model, dev_data, tokenizer, device, args):
    model.eval()
    generated_responses = []
    reference_answers = []
...
if __name__ == '__main__':
    ...
    # Validate the model
    validate_model(model, dev_data, tokenizer, device, args)
