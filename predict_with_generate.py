...
def create_data(data, tokenizer, max_len):
    ...
    for item in data:
        content = item[0]  # 从元组中获取字符串
        if len(item) > 1:
            title = item[1]
        else:
            title = None  # 将用户输入的问题的 title 设置为 None
...
def prepare_data(args, tokenizer):
    ...
    user_data = [(user_input,)]  # 注意这里将用户输入的问题放入元组中

    user_data = create_data(user_data, tokenizer, args.max_len)
    user_data = KeyDataset(user_data)
    user_data = DataLoader(user_data, batch_size=args.batch_size, collate_fn=default_collate)
    return user_data
...
def init_argument():
    ...
    parser.add_argument('--model', default='./caoling_QA_model/QAmodel')

    parser.add_argument('--batch_size', default=16, help='batch size')
    parser.add_argument('--max_len', default=25, help='max length of inputs')
    parser.add_argument('--max_len_generate', default=350, help='max length of generated text')
...
# 配置日志
log_path = '195epoch_2e-4_QA.log'  # 替换成实际的日志文件路径
logging.basicConfig(filename=log_path, level=logging.INFO)

# 创建日志对象
logger = logging.getLogger(__name__)

# def generate(user_data, model, tokenizer, args):
#     gens, summaries = [], []
#     model.eval()
#     for feature in tqdm(user_data):
#         raw_data = feature['raw_data']
...
#                              **content)
#         gen = tokenizer.batch_decode(gen, skip_special_tokens=True)
#         gen_str = gen[0].replace(' ', '')  # 不限制输出长度
#         print("Generated Text:", gen_str)
#         gens.extend([gen_str])
#
#     print('Done!')
...
def generate(user_data, model, tokenizer, args):
    ...
    for feature in tqdm(user_data):
        ...
        logger.info(f"Generated Text: {gen_str}")
...
if __name__ == '__main__':
    ...
    # step 2. prepare user input data
    tokenizer = T5PegasusTokenizer.from_pretrained(args.pretrain_model)
    user_data = prepare_data(args, tokenizer)

    # step 3. load finetuned model
    model = torch.load(args.model, map_location=device)

    # step 4. predict
    generate(user_data, model, tokenizer, args)
